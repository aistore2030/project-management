<?php
add_action( 'admin_menu' , 'register_pto_messaging_admin_page', 10 ); 
function register_pto_messaging_admin_page() {
	$mypage = add_submenu_page(	
				'pto-dashboard',
				__('All Messages (Admin)', 'cqpim'),		
				__('All Messages (Admin)', 'cqpim'), 			
...
function pto_messages_admin() {
	...
	if(user_can($user->ID, 'cqpim_message_clients_from_projects')) {
		$text = __('Search for team member name, client name or client company...', 'cqpim');
	}
	...
	?>
	<br />
	<div class="cqpim-dash-item-full" style="padding-right:10px">
		<div id="cqpim-new-message" class="cqpim_block" style="display:none">
			<div class="cqpim_block_title">
				...
				<div class="actions">
					<button id="send" class="cqpim_button cqpim_small_button border-green font-green rounded_2 op"><i class="fa fa-paper-plane" aria-hidden="true"></i> <?php _e('Send', 'cqpim'); ?></button>	
			...
			<form id="cqpim-create-new-message">
				...
				<p><span class="cqpim-heading"><?php _e('Message:', 'cqpim'); ?></span><textarea id="cmessage" name="cmessage" /></textarea></p>
				<div class="clear"></div>
		...
		<div id="cqpim-reply-message" class="cqpim_block" style="display:none">
			<div class="cqpim_block_title">
				...
				<div class="actions">
					<button id="send-reply" class="cqpim_button cqpim_small_button border-green font-green rounded_2 op" data-conversation="<?php echo isset($conversation->ID) ? $conversation->ID : ''; ?>"><i class="fa fa-paper-plane" aria-hidden="true"></i> <?php _e('Send', 'cqpim'); ?></button>	
		...
		<div class="cqpim_block">
			<div class="cqpim_block_title">
				<div class="caption">
					...
					<?php if(!empty($conversation)) { ?>
						<span class="caption-subject font-green-sharp sbold"><?php echo get_the_title($conversation->ID); ?></span>
					<?php } else { ?>
						<span class="caption-subject font-green-sharp sbold"><?php _e('All Messages (Admin)', 'cqpim'); ?></span>
					<?php } ?>
				</div>
				<div class="actions">
			...
			<?php } else { ?>
				...
				if(!empty($conversation)) {
					...
					if(!in_array($user->ID, $recipients) && !current_user_can('access_cqpim_messaging_admin')) {
					...
					<div id="cqpim-messaging-buttons">
						<button id="cqpim-edit-subject" class="cqpim_button cqpim_small_button left font-white bg-amber rounded_2 op"><i class="fa fa-pencil-square-o" aria-hidden="true"></i><span class="desktop_only"> <?php _e('Edit Subject', 'cqpim'); ?></span></button>
						<button id="cqpim-cancel-edit-subject" class="cqpim_button cqpim_small_button left font-white bg-red rounded_2 op"><i class="fa fa-times" aria-hidden="true"></i><span class="desktop_only"> <?php _e('Cancel', 'cqpim'); ?></span></button>
						<button id="cqpim-save-subject" class="cqpim_button cqpim_small_button left font-white bg-green rounded_2 op"><i class="fa fa-floppy-o" aria-hidden="true"></i><span class="desktop_only"> <?php _e('Save', 'cqpim'); ?></span></button>
						...
						<?php } ?>
						<button id="cqpim-convo-add" class="cqpim_button cqpim_small_button font-white bg-blue rounded_2 op right"><i class="fa fa-address-book" aria-hidden="true"></i><span class="desktop_only"> <?php _e('Add User', 'cqpim'); ?></span></button>
						<button id="cqpim-convo-remove" class="cqpim_button cqpim_small_button font-white bg-red rounded_2 op right"><i class="fa fa-times" aria-hidden="true"></i><span class="desktop_only"> <?php _e('Remove User', 'cqpim'); ?></span></button>
						<div class="clear"></div>
					</div>
				...
				<?php } else { ?>
					<table class="dataTable datatable_style milestones" data-ordering="[[ 4, 'desc' ]]" data-rows="10">
						<thead>
							<tr>
						...
						<tbody>
							<?php foreach($conversations as $conversation) { 
								...
								<tr<?php if(empty($read_val)) { echo ' class="cqpim-unread"'; } ?>>
									<td><?php if(empty($read_val)) { echo '<i class="fa fa-envelope" aria-hidden="true"></i> '; } else { echo '<i class="fa fa-envelope-open" aria-hidden="true"></i> '; } ?>&nbsp;&nbsp;<a href="<?php echo admin_url() . 'admin.php?page=pto-messages&conversation=' . $id; ?>"><?php echo get_the_title($conversation->ID); ?></a></td>
