<?php
add_action( 'admin_menu' , 'register_pto_task_page', 9 ); 
	...
	<div class="cqpim_block">
		<div class="cqpim_block_title">
			<div class="caption">
				...
				<span class="caption-subject font-green-sharp sbold"><?php _e('My Tasks', 'cqpim'); ?> </span>
			</div>
			<div class="actions">
		...
		if($tasks) { ?>
			...
			<table class="datatable_style dataTable">
				<thead>
					...
					<th><?php _e('Project', 'cqpim'); ?></th>
					<th><?php _e('Assigned', 'cqpim'); ?></th>
					<th><?php _e('Deadline', 'cqpim'); ?></th>
					<th><?php _e('Timer', 'cqpim'); ?></th>
				...
				<tbody>
					<?php foreach($ordered as $task) { 
						...
						<tr<?php if(pto_is_task_overdue($task->ID) == 1) { ?> class="overdue"<?php } ?>>
							...
							<td><button class="add_timer legacy_button border-green font-green cqpim_button cqpim_xs_button op" value="<?php echo $task->ID; ?>" data-title="<?php echo $task->post_title; ?>"><i class="fa fa-clock-o" aria-hidden="true" title="<?php _e('Add Time', 'cqpim'); ?>"></i></button></td>
