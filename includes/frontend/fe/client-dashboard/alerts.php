<div class="cqpim_block">
	<div class="cqpim_block_title">
		<div class="caption">
			<i class="fa fa-exclamation-triangle font-green-sharp" aria-hidden="true"></i> <?php _e('Alerts', 'cqpim'); ?>
		</div>
	</div>
	<div class="cqpim-dash-item-inside">
		...
		if(!empty($messages)) {
			$alerts[] = array(
				'type' => 'message',
				'level' => 'info',
				'data' => __('You have new messages', 'cqpim') . ' - <a style="color:#222" href="?page=messages">' . __('View', 'cqpim') . '</a>',
			);					
		}
			...
			foreach($bugs as $bug) {
				...
				if(!empty($updated) && !empty($client_ids) && in_array($user->ID, $client_ids)) {
					$alerts[] = array(
						'type' => 'message',
						'level' => 'info',
						'data' => sprintf(__('You have an updated bug: %1$s', 'cqpim'), $bug->post_title) . ' <a href="' . get_the_permalink($bug->ID) . '"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i></i></a>',
					);
				}
			}
		}
