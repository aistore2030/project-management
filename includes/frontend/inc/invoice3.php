...
<body style="background:#fff" <?php body_class(); ?>>
	<div style="background: #fff;margin: 0px auto;width: 800px;padding:0;text-align: left" id="content" role="main">
		<?php if(current_user_can( 'edit_cqpim_invoices' ) OR $client_user_id == $user_id OR $pass == md5($post->post_password) OR in_array($user->ID, $client_ids)) { ?>	
			<div style="padding:50px">		
				<div style="width:200px; margin:0; float:left; padding:0">
				...
				<div style="margin:0; float:right; padding:10px 00 0">	
					<div style="border-bottom:5px solid <?php echo $main_colour; ?>; float:right; width:100px">
						<span style="font-size:12px"><?php _e('Total:', 'cqpim'); ?></span><br />
						<span style="font-size:18px; font-weight:bold"><?php echo pto_calculate_currency($post->ID, $total); ?></span>
					</div>
					<div style="width:30px; float:right">
						<br />
					</div>
					<div style="border-bottom:5px solid <?php echo $main_colour; ?>; float:right; width:100px">
						<span style="font-size:12px"><?php _e('Invoice #:', 'cqpim'); ?></span><br />
					...
					<div style="border-bottom:5px solid <?php echo $main_colour; ?>; float:right; width:120px">
						<span style="font-size:12px"><?php _e('Invoice Date:', 'cqpim'); ?></span><br />
						<span style="font-size:18px; font-weight:bold"><?php echo $invoice_date; ?></span>
					</div>
					<div style="clear:both"></div>
				</div>		
				<div class="clear"></div>
				<div style="text-align:center; padding:20px 0 30px; font-size:28px; font-weight:100">
					<?php _e('INVOICE', 'cqpim'); ?>
				</div>
				<div style="float:left; background:<?php echo $main_colour; ?>; padding:30px; width:240px">
					<span style="color:#fff;" class="invoice_to">
						<span style="color:#fff; font-size:14px; font-weight:bold"><?php _e('INVOICE TO:', 'cqpim'); ?></span><br />
						...
						<?php if(!empty($client_tax_name)) { ?>
							<?php if(!empty($client_tax_name)) { ?>
							<span style="color:#fff; font-size:12px; font-weight:bold"><?php printf(__('%1$s Reg Number: ', 'cqpim'), $client_tax_name); ?></span> <?php echo $client_tax_reg; ?><br />
							<?php } ?>
							<?php if(!empty($client_stax_name)) { ?>
								<span style="color:#fff; font-size:12px; font-weight:bold"><?php printf(__('%1$s Reg Number: ', 'cqpim'), $client_stax_name); ?></span> <?php echo $client_stax_reg; ?><br />
							<?php } ?>
						<?php } ?>
					</span>
				</div>
				<div style="float:right; padding:30px 30px 30px 0; width:270px">
					<span class="invoice_to">
						...
						<span style="color:<?php echo $main_colour; ?>; font-size:14px; font-weight:bold"><?php echo get_option('secondary_sales_tax_name'); ?> <?php _e('Number', 'cqpim'); ?>:</span> <span style="font-size:12px"><?php echo $stax_reg; ?></span><br />						
							<?php } ?>
						<span style="color:<?php echo $main_colour; ?>; font-size:14px; font-weight:bold"><?php _e('Due Date:', 'cqpim'); ?></span>						
				...
				<table style="width:100%; border: 0; border-collapse:collapse; margin:0">
					<thead>
						<tr>
							<th style="background:<?php echo $main_colour; ?>; border:0; color:#fff; padding:16px; font-weight:300; font-size:16px; text-align:center"><?php _e('Qty', 'cqpim'); ?></th>
							<th style="background:<?php echo $main_colour; ?>; border:0; color:#fff; padding:16px; font-weight:300; font-size:16px"><?php _e('Description', 'cqpim'); ?></th>
							<th style="width:120px; background:<?php echo $main_colour; ?>; border:0; color:#fff; padding:16px; font-weight:300; font-size:16px"><?php _e('Rate', 'cqpim'); ?></th>
							<th style="width:120px; background:<?php echo $main_colour; ?>; border:0; color:#fff; padding:16px; font-weight:300; font-size:16px"><?php _e('Total', 'cqpim'); ?></th>
						</tr>
					</thead>
					<tbody>
					<?php 
					...
					foreach($line_items as $item) { ?>
						<tr>
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px; text-align:center"><?php echo $item['qty']; ?></td>
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo $item['desc']; ?></td>
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $item['price']); ?></td>
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $item['sub']); ?></td>
						</tr>
					<?php } ?>
						<tr>
							...
							<td style="background:#fff; border:0; color:<?php echo $main_colour; ?>; padding:10px; font-weight:300; font-size:14px; text-align:right; font-weight:bold"><?php if($vat_rate) { ?><?php _e('Subtotal:', 'cqpim'); ?><?php } else { ?><?php _e('TOTAL:', 'cqpim'); ?><?php } ?></td>
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $sub); ?></td>
						</tr>
					<?php 
					$outstanding = $sub;
					if($vat_rate) { 
						...
						<tr>
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $vat); ?></td>
						</tr>
						<?php if(!empty($svat_rate)) { ?>
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $svat); ?></td>
							</tr>					
						<?php } ?>
						<tr>
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $total); ?></td>
						</tr>
					<?php } ?>
						<tr>
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $received); ?></td>
						</tr>
						<tr>
							...
							<td style="background:#fff; border:0; border-bottom:1px solid #ececec; color:#000; padding:10px; font-weight:300; font-size:14px"><?php echo pto_calculate_currency($post->ID, $outstanding); ?></td>
						</tr>
					</tbody>
				</table>
				<br />
				<div style="float:left; padding:20px; width:180px; background:#fff">
					<span style="color:<?php echo $main_colour; ?>"><?php echo $company_address; ?><br /><?php echo $company_postcode; ?></span>
				</div>
				<div style="height: 76px; float:right; padding:20px; width:420px; background:#fff">
					<span style="color:<?php echo $main_colour; ?>"><?php _e('Tel: ', 'cqpim'); ?> <?php echo $company_telephone; ?></span><br />
					<span style="color:<?php echo $main_colour; ?>"><?php _e('Email: ', 'cqpim'); ?></span> <a style="color:<?php echo $main_colour; ?>" href="mailto:<?php echo $company_accounts_email; ?>"><?php echo $company_accounts_email; ?></a>
				</div>
				<div class="clear"></div>
				<br /><br />			
			</div>
