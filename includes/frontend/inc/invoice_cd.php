...
<div class="cqpim-dash-content">
	...
	<div id="cqpim-dash-content">
		<div id="cqpim_admin_title">
			...
			$title = get_the_title(); $title = str_replace('Protected:', '', $title); echo '<a href="' . get_the_permalink($client_dash) . '">' . __('Dashboard', 'cqpim') . '</a> <i class="fa fa-circle"></i> <a href="' . get_the_permalink($client_dash) . '?page=invoices">' . __('Invoices', 'cqpim') . '</a> <i class="fa fa-circle"></i> ' . $title;
			} else {
			...
			<div class="masonry-grid">
				...
				<div class="cqpim-dash-item-double grid-item">
					<div class="cqpim_block">
						...
						<a class="cqpim_button cqpim_button_link font-white bg-blue block mt-10 rounded_2 block" href="<?php echo get_the_permalink(); ?>?download_pdf=1"><?php _e('Download PDF Invoice', 'cqpim') ; ?></a>
					</div>
				</div>		
			</div>
			<?php if(!empty($ideal)) { 
