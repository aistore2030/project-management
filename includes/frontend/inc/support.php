	...
	<div id="cqpim-dash-content">
		<div id="cqpim_admin_title">
			...
			if($post->post_author == $user_id OR $assigned == $ticket_client) { 
				$title = get_the_title(); $title = str_replace('Private:', '', $title); echo '<a href="' . get_the_permalink($client_dash) . '">' . __('Dashboard', 'cqpim') . '</a> <i class="fa fa-circle"></i> <a href="' . get_the_permalink($client_dash) . '?page=support">' . __('Support Tickets', 'cqpim') . '</a> <i class="fa fa-circle"></i> '; echo $title; 
			} else {
					...
					<div class="cqpim-dash-item-triple grid-item">
						<div id="ticket_container" class="cqpim_block">
							...
							if(empty($ticket_updates)) {
								...
								echo '<p>' . __('There are no updates on this ticket.', 'cqpim') . '</p>';
							} ?>
