<?php
function register_pto_session() {
	if( !session_id() ) {
		session_start();
	}
}
add_action('init', 'register_pto_session');
function pto_get_user_role( $user = null ) {
	$user = $user ? new WP_User( $user ) : wp_get_current_user();
	$roles = $user->roles;
	$role = isset($roles[0]) ? $roles[0] : __('No Role', 'cqpim');
...
function pto_check_addon_status($addon) {	
	...
	if($addon == 'envato') {
		if(is_plugin_active('pto-envato/pto-envato.php') || defined('PTO_ENVATO_DIRNAME') && is_plugin_active(PTO_ENVATO_DIRNAME . '/pto-envato.php')) {
			return true;
		}
	}
	if($addon == '2checkout') {
...
add_filter( 'cron_schedules', 'pto_schedule_minute' );
...
add_filter( 'post_row_actions', 'pto_remove_row_actions', 39, 1 );
function pto_remove_row_actions( $actions ) {
	...
	if( get_post_type() === 'cqpim_invoice' ) {
		unset( $actions['view'] );
		unset($actions['inline hide-if-no-js']);
	}
	...
	if( get_post_type() === 'cqpim_plan' ) {
		unset( $actions['view'] );
		unset($actions['inline hide-if-no-js']);
	}
	if( get_post_type() === 'cqpim_subsription' ) {
		unset( $actions['view'] );
		unset( $actions['trash'] );
		unset($actions['inline hide-if-no-js']);
	}
	if( get_post_type() === 'cqpim_lead' ) {
		unset( $actions['view'] );
		unset( $actions['trash'] );
		unset($actions['inline hide-if-no-js']);
	}
	if( get_post_type() === 'cqpim_leadform' ) {
		unset( $actions['view'] );
		unset( $actions['trash'] );
		unset($actions['inline hide-if-no-js']);
	}
	if( get_post_type() === 'cqpim_faq' ) {
		unset( $actions['view'] );
		unset( $actions['trash'] );
		unset($actions['inline hide-if-no-js']);
	}
...
function pto_upload_user_file( $file = array(), $project_id ) {
...
function pto_calculate_currency($post_id, $amount = '') {
	...
	if(empty($amount)) {
		...
		if($post_currency_position == 'l') {
			return $post_currency . $space . "";
		} else {
			return "" . $space . $post_currency;
		}
...
function pto_send_emails($to, $subject, $message, $headers = null, $attachments = array(), $type) {
	$headers = array();
...
function pto_replacement_patterns($content, $post_id, $type = NULL) {
	...
	if($type == 'invoice') {		
		...
		$due = date(get_option('cqpim_date_format'), $due);
	...
	if($type == 'subscription') {
		...
		if($subscription_frequency == 'day') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Daily', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'week') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Weekly', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'biweek') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Every Two Weeks', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'month') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Monthly', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'bimonth') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Every Two Months', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'sixmonth') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Every 6 Months', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'year') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Yearly', 'cqpim-subscriptions') . '</p>';
		} elseif($subscription_frequency == 'biyear') {
			$table_content .= '<p><strong>' . __('Frequency of Billing', 'cqpim-subsriptions') . '</strong> : ' . __('Every Two Years', 'cqpim-subscriptions') . '</p>';
		}
		if(!empty($sub_item_type) && $sub_item_type == 'plan') {
			...
			$table_content .= '<table class="cqpim_table" style="width:100%">';
				$table_content .= '<thead style="background:#ececec">';
					$table_content .= '<tr>';
						$table_content .= '<th>' . __('Qty', 'cqpim-subscriptions') . '</th>';
						$table_content .= '<th>' . __('Description', 'cqpim-subscriptions') . '</th>';
						$table_content .= '<th>' . __('Rate', 'cqpim-subscriptions') . '</th>';
						$table_content .= '<th>' . __('Total', 'cqpim-subscriptions') . '</th>';
					$table_content .= '</tr>';
				$table_content .= '</thead>';
				$table_content .= '<tbody>';
				if(empty($line_items)) {
				...
				$table_content .= '<tr>';
					...
					if($vat_rate) { 
						$table_content .= __('Subtotal:', 'cqpim-subscriptions');
					} else { 
						$table_content .= __('TOTAL:', 'cqpim-subscriptions');
					}
					$table_content .= '</td>';
				...
				if($vat_rate) { 
					...
					$tax_name = get_option('sales_tax_name');
					$table_content .= '<tr>';
						$table_content .= '<td style="text-align:right; font-weight:bold" colspan="3">' . $tax_name . ':</td>';
						$table_content .= '<td style="text-align:center">' . pto_calculate_currency($post_id, $vat) . '</td>';
					$table_content .= '</tr>';
					if(!empty($svat_rate)) {
						$table_content .= '<tr>';
						$table_content .= '<td style="text-align:right; font-weight:bold" colspan="3">' . $stax_name . ':</td>';
						$table_content .= '<td style="text-align:center">' . pto_calculate_currency($post_id, $svat) . '</td>';
						$table_content .= '</tr>';				
					}
					$table_content .= '<tr>';
						$table_content .= '<td style="text-align:right; font-weight:bold" colspan="3">' . __('TOTAL:', 'cqpim-subscriptions') . '</td>';
...
add_action( "wp_ajax_pto_switch_custom_field_type", "pto_switch_custom_field_type");
function pto_switch_custom_field_type() {
	$type = isset($_POST['type']) ? $_POST['type'] : '';
	$_SESSION['cqpim_cf_post_type'] = $type;
	$return =  array( 
		'error' 	=> false,
		'errors' => 'none'
	);
