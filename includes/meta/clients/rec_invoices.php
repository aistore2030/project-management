...
function pto_client_recinvoices_metabox_callback( $post ) {
 	wp_nonce_field( 
	'client_recinvoices_metabox', 
	'client_recinvoices_metabox_nonce' ); ?>
	<p><?php _e('Recurring Invoices can be automatically created and sent on specified intervals.', 'cqpim'); ?></p>
	...
	if(empty($recurring_invoices)) {
		...
		echo '<table class="datatable_style dataTable" data-sort="false" data-rows="10">';
		echo '<thead>';
		echo '<tr><th>' . __('Title', 'cqpim') . '</th><th>' . __('Status', 'cqpim') . '</th><th>' . __('Start', 'cqpim') . '</th><th>' . __('End', 'cqpim') . '</th><th>' . __('Frequency', 'cqpim') . '</th><th>' . __('Last Issue', 'cqpim') . '</th><th>' . __('Next Issue', 'cqpim') . '</th><th>' . __('Actions', 'cqpim') . '</th></tr>';
		echo '</thead>';
		echo '<tbody>';
		foreach($recurring_invoices as $key => $invoice) {
			...
			<tr>
				...
				<td><span class="cqpim_mobile"><?php _e('Start:', 'cqpim'); ?></span> <?php echo $start; ?></td>
				<td><span class="cqpim_mobile"><?php _e('End:', 'cqpim'); ?></span> <?php echo $end; ?></td>
				<td><span class="cqpim_mobile"><?php _e('Frequency:', 'cqpim'); ?></span> <?php echo ucfirst(isset($invoice['frequency']) ? $invoice['frequency'] : ''); ?></td>
				<td><span class="cqpim_mobile"><?php _e('Last Issue:', 'cqpim'); ?></span> <?php echo isset($invoice['last_run']) ? date(get_option('cqpim_date_format'), $invoice['last_run']) : 'N/A'; ?></td>
				<td><span class="cqpim_mobile"><?php _e('Next Issue:', 'cqpim'); ?></span> <?php echo $next; ?></td>
				<td><?php if($invoice['status'] == 1) { ?><button class="edit-task cqpim_button cqpim_small_button font-amber border-amber" value="<?php echo $key; ?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button> <?php } ?><button class="delete_task cqpim_button cqpim_small_button font-red border-red" value="<?php echo $key; ?>"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
			</tr>
		<?php }
		echo '</tbody>';
		echo '</table>';
	...
	<div style="display:none" id="edit-recurring-invoice-container-<?php echo $key; ?>">
		<div id="edit-recurring-invoice-<?php echo $key; ?>" class="edit-rec-inv" style="width:830px">
			<div style="padding:12px">
				...
				<button class="edit-rec-inv-btn cqpim_button font-green border-green op right" value="<?php echo $post->ID; ?>" data-key="<?php echo $key; ?>"><?php _e('Edit', 'cqpim'); ?><span id="edit-rec-inv-spinner-<?php echo $key; ?>" class="ajax_loader" style="display:none"></span></button>
				</div>
				<div class="clear"></div>
			</div>
		</div>
	...
	<?php } ?>		
	<button class="mt-20 cqpim_button cqpim_small_button border-blue font-blue right" id="create-rec-inv"><?php _e('Add Recurring Invoice', 'cqpim'); ?></button>
	<div class="clear"></div>
	<div style="display:none" id="add-recurring-invoice-container">
		<div id="add-recurring-invoice" style="width:830px">
			<div style="padding:12px">
				<h3><?php _e('Add Recurring Invoice', 'cqpim'); ?></h3>
				<div style="width:800px">
					<div style="float:left; width:390px">
					<p><strong><?php _e('Title', 'cqpim'); ?></strong></p>
					<input type="text" id="rec-inv-title" value="" />
					<p><strong><?php _e('Contact', 'cqpim'); ?></strong></p>
					<select id="client_contact_select">
						<?php 
						$client_details = get_post_meta($post->ID, 'client_details', true);
						...
						echo '<option value="' . $user_id . '">' . $client_contact . ' ' . __('(Main contact)', 'cqpim') . '</option>';
						$contacts = get_post_meta($post->ID, 'client_contacts', true);
						$contacts = $contacts&&is_array($contacts)?$contacts:array();
						foreach($contacts as $contact) {
							echo '<option value="' . $contact['user_id'] . '">' . $contact['name'] . '</option>';
						}
						?>
					</select>
					<p><strong><?php _e('Start/End', 'cqpim'); ?></strong>  <i class="fa fa-question-circle cqpim_tooltip" aria-hidden="true" title="<?php _e('You can specify a Start/End date for this invoice. If you leave the start blank then the invoice will start sending today. If you leave End blank, then the invoice will send according to the frequency until you delete or disable it.', 'cqpim'); ?>"></i></p>
					<input style="width:48%; margin-right:2%" type="text" class="datepicker" id="rec-inv-start" placeholder="<?php _e('Start', 'cqpim'); ?>" />
					<input style="width:48%;" type="text" class="datepicker" id="rec-inv-end" placeholder="<?php _e('End', 'cqpim'); ?>" />
					<div style="float:left; width:51%">
						<p><strong><?php _e('Frequency', 'cqpim'); ?></strong></p>
						<select id="rec-inv-frequency">
							<option value=""><?php _e('Choose a frequency', 'cqpim'); ?></option>
							<option value="daily"><?php _e('Daily', 'cqpim'); ?></option>
							<option value="weekly"><?php _e('Weekly', 'cqpim'); ?></option>
							<option value="biweekly"><?php _e('Fortnightly', 'cqpim'); ?></option>
							<option value="monthly"><?php _e('Monthly', 'cqpim'); ?></option>
							<option value="bimonthly"><?php _e('Bi Monthly', 'cqpim'); ?></option>
							<option value="threemonthly"><?php _e('Every Three Months', 'cqpim'); ?></option>
							<option value="sixmonthly"><?php _e('Every Six Months', 'cqpim'); ?></option>
							<option value="yearly"><?php _e('Yearly', 'cqpim'); ?></option>
							<option value="biyearly"><?php _e('Biyearly', 'cqpim'); ?></option>
						</select>
					</div>
					<div style="float:left; width:48%">
						<p><strong><?php _e('Status', 'cqpim'); ?></strong></p>
						<select id="rec-inv-status">
							<option value="1"><?php _e('Active', 'cqpim'); ?></option>
							<option value="0"><?php _e('Disabled', 'cqpim'); ?></option>
						</select>
					</div>
					<div class="clear"></div>
					<br />
					<input type="checkbox" id="rec-inv-auto" /> <?php _e('Send the recurring invoices to the client on creation.', 'cqpim'); ?><br />
					<input type="checkbox" id="rec-inv-partial"/> <?php _e('Allow partial payments on this invoice.', 'cqpim'); ?>
					</div>
					<div style="float:right; width:390px">
					<p><strong><?php _e('Invoice Items', 'cqpim'); ?></strong></p>
					<div class="repeater">	
					...
					<div class="clear"></div>
					<button class="cancel-colorbox cqpim_button font-red border-red op"><?php _e('Cancel', 'cqpim'); ?></button>
					<button id="add_rec_inv" class="cqpim_button font-green border-green op right" value="<?php echo $post->ID; ?>"><?php _e('Add Recurring Invoice', 'cqpim'); ?> <span id="rec-inv-spinner" class="ajax_loader" style="display:none"></span></button>
					</div>
					<div class="clear"></div>
				</div>
			</div>
