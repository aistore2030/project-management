<?php
function pto_invoice_currency_metabox_callback( $post ) {
 	wp_nonce_field( 
	'invoice_currency_metabox', 
	'invoice_currency_metabox_nonce' );
	$quote_details = get_post_meta($post->ID, 'invoice_details', true);
	...
	?>
	<p><?php _e('If you would like to override the currency on this invoice you can do so here. By default the invoice will use the system currency settings, however if the client that the invoice is assigned to has custom settings these will be used instead.', 'cqpim'); ?></p>
	...
	<table style="width:100%">
		<tr>
			<td>
				<?php _e('Invoice Currency Symbol:', 'cqpim'); ?> <br />
				<input type="text" name="currency_symbol" value="<?php echo $quote_currency; ?>" />
			</td>
		</tr>
		<tr>
			<td>
				<?php _e('Invoice Currency Code:', 'cqpim'); ?> <br />				
		...
		<tr>
			<td>
				<?php _e('Invoice Currency Symbol Position: ', 'cqpim'); ?> <br />
...
add_action( 'save_post', 'save_pto_invoice_currency_metabox_data' );
function save_pto_invoice_currency_metabox_data( $post_id ){
	if ( ! isset( $_POST['invoice_currency_metabox_nonce'] ) )
	    return $post_id;
	$nonce = $_POST['invoice_currency_metabox_nonce'];
	if ( ! wp_verify_nonce( $nonce, 'invoice_currency_metabox' ) )
	    return $post_id;
