...
function pto_lead_files_metabox_callback( $post ) {
 	wp_nonce_field( 
	'lead_files_metabox', 
	'lead_files_metabox_nonce' );
	$all_attached_files = get_attached_media( '', $post->ID );
	if(!$all_attached_files) {
		echo '<p>' . __('There are no files attached to this lead.', 'cqpim') . '</p>';
	} else {
...
add_action( 'save_post', 'save_pto_lead_files_metabox_data' );
function save_pto_lead_files_metabox_data( $post_id ){
	if ( ! isset( $_POST['lead_files_metabox_nonce'] ) )
	    return $post_id;
	$nonce = $_POST['lead_files_metabox_nonce'];
