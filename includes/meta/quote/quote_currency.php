...
function pto_quote_currency_metabox_callback( $post ) {
 	wp_nonce_field( 
	'quote_currency_metabox', 
	'quote_currency_metabox_nonce' );
	...
	?>
	<p><?php _e('If you would like to override the currency on this quote you can do so here. By default the quote will use the system currency settings, however if the client that the quote is assigned to has custom settings these will be used instead.', 'cqpim'); ?></p>
	...
	<table style="width:100%">
		<tr>
			<td>
				<?php _e('Quote Client Currency Symbol:', 'cqpim'); ?> <br />
				<input type="text" name="currency_symbol" value="<?php echo $quote_currency; ?>" />
			</td>
		</tr>
		<tr>
			<td>
				<?php _e('Quote Currency Code:', 'cqpim'); ?> <br />				
		...
		<tr>
			<td>
				<?php _e('Quote Currency Symbol Position: ', 'cqpim'); ?> <br />
...
add_action( 'save_post', 'save_pto_quote_currency_metabox_data' );
function save_pto_quote_currency_metabox_data( $post_id ){
	if ( ! isset( $_POST['quote_currency_metabox_nonce'] ) )
	    return $post_id;
	$nonce = $_POST['quote_currency_metabox_nonce'];
	if ( ! wp_verify_nonce( $nonce, 'quote_currency_metabox' ) )
	    return $post_id;
