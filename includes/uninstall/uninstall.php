...
function pto_deactivation() {
	if(pto_check_addon_status('envato') || pto_check_addon_status('2checkout') || pto_check_addon_status('expenses') || pto_check_addon_status('reporting') || pto_check_addon_status('bugs') || pto_check_addon_status('subscriptions') || pto_check_addon_status('woocommerce')) {
		$message = __('You cannot disable Projectopia because some add-ons are active. Please disable those first.', 'cqpim');
		wp_die($message, 'Projectopia Add-Ons Are Active');		
	}
...
add_action( "wp_ajax_pto_remove_all_data", 
		"pto_remove_all_data");
